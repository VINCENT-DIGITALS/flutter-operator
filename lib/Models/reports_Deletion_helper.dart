...
class ReportsDeletionHelper {
  ...
  static Future<void> _deleteAllDocuments(BuildContext mainContext) async {
    ...
    final QuerySnapshot snapshot = await _firestore.collection('reports').get();

    if (snapshot.docs.isEmpty) {
      Fluttertoast.showToast(msg: 'No records found to delete.');
      Navigator.of(mainContext)
          .pop(); // Close loading dialog if nothing to delete
      return;
    }
  ...
  static Future<void> initialConfirmation(BuildContext mainContext) async {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
            'This will permanently delete all citizens reports records. Do you want to continue?'),
        actions: [
          TextButton(
          ...
          TextButton(
            onPressed: () {
              ...
              confirmAndDeleteLogBook(
                  mainContext); // Pass main context to avoid invalidation
            },
