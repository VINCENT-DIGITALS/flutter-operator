  ...
  Future<void> _createPost() async {
    if (_formKey.currentState!.validate()) {
      ...
      for (int i = 0; i < _selectedFiles.length; i++) {
        String fileUrl = await _uploadFileWithProgress(_selectedFiles[i], 'posts', _fileTypes[i]);
        fileUrls.add(fileUrl);
        fileTypes.add(_fileTypes[i]);
      }

      for (int i = 0; i < _webImages.length; i++) {
        String fileUrl = await _uploadWebImageWithProgress(_webImages[i], 'posts', _fileTypes[i]);
        fileUrls.add(fileUrl);
        fileTypes.add(_fileTypes[i]);
      }
      ...
      await _dbService.addPost(postData);
      print('Post created: $postData');
