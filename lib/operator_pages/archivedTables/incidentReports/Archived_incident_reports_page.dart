...
import 'Archived_Report_download_options_dialog.dart';
import 'Archived_incident_report_dataTable.dart';
import 'Archived_reportsArchive_helper.dart'; // Import CustomDrawer

class Archived_IncidentReportManagementPage extends StatefulWidget {
  const Archived_IncidentReportManagementPage({super.key});

  @override
  _Archived_IncidentReportManagementPageState createState() =>
      _Archived_IncidentReportManagementPageState();
}

class _Archived_IncidentReportManagementPageState
    extends State<Archived_IncidentReportManagementPage> {
  ...
  void _showDownloadDialog() {
    showDialog(
      context: context,
      builder: (context) => Archived_ReportDownloadDialog(),
    );
  }
      ...
      builder: (context, constraints) {
        ...
        return Scaffold(
          ...
          appBar: CustomAppBar(
            ...
            title: 'Archived Report Managements',
          ),
          drawer: isLargeScreen
              ? null
              : CustomDrawer(
                  scaffoldKey: _scaffoldKey, currentRoute: '/Archived_incident_reports'),
          body: Row(
            children: [
              if (isLargeScreen)
                Container(
                  ...
                  child: CustomDrawer(
                      scaffoldKey: _scaffoldKey,
                      currentRoute: '/Archived_incident_reports'),
                ),
              Expanded(
                child: Padding(
                                        ...
                                        children: [
                                          IconButton(
                                            icon: Icon(Icons.download,
                                                color: Colors.blue),
                                                  tooltip: 'Download Archived Citizen Reports',
                                            onPressed: _showDownloadDialog,
                                          ),
                                          IconButton(
                                            icon: Icon(Icons.delete_forever,
                                                color: Colors.red),
                                                  tooltip: 'Unarchived Citizen Reports',
                                            onPressed: () {
                                              ArchivedReportsArchivalHelper
                                ...
                                child:
                                  ...
                                  builder: (context, snapshot) {
                                    if (snapshot.connectionState ==
                                      ...
                                      return Center(
                                          child: Text(
                                              'No Archived Citizens reports record available!'));
                                    } else {
  ...
  Widget _buildIncidentReportsTable(
    ...
    if (tabType == 'Pending') {
      filteredData = data.where((item) {
        ...
        final isArchived =
            item['archived']; // Default to false if null
      ...
      filteredData = data.where((item) {
        final isArchived = item['archived'];
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          return PaginatedDataTable(
            columns: columns,
            source: ArchivedIncidentReportDataTableSource(
